  ...
  return (
    <Container detail={detail} ref={slideShow}>
      {detail && amountImages > 1 && (
        <Button right="true" onClick={() => nextImage()}>
          {'>'}
        </Button>
      )}
      {detail && amountImages > 1 && <Button onClick={() => afterImage()}>{'<'}</Button>}
      {amountImages > 1 && detail
        ? data.map((item, index) => {
            return (
              <DivImg key={index} divActive={index === selectedImage ? 'active' : 'inactive'}>
                {selectedImage === index && <Img src={BASE_URL + item} alt="inmobiliaria" />}
              </DivImg>
            );
          })
