...
export const AdminCountriesComponent = ({data, setData, handleFetch}) => {
    ...
    const [errorInput, setErrorInput] = useState({
        ...
        citys: data.citys.map((item, index)=>{return{[index]: false}})
    })
    ...
    const handleErrorLongitudAndLatitud= (nameInput) =>{
        ...
        let hasError = false;
        values.forEach((item)=>{
            const validateOne = item === 'value1' ? information[item].length > 3 : information[item].length > 2
    ...
    const handleTransformData = () => {
        ...
        const citysMayus = cityData.map((item)=>item[0].toUpperCase() + item.substr(1))
        const dataFetch = {
    ...
    return (
        ...
        <>
            ...
            {viewModals.return &&  <ModalComponent
                                title="¡Cuidado!"
